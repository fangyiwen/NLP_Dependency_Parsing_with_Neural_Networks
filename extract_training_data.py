...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        stack_three = state.stack[-1:-4:-1]
        buffer_three = state.buffer[-1:-4:-1]

        for i in range(len(stack_three)):
            word = words[stack_three[i]]
            pos_word = pos[stack_three[i]]
            if word is None:
                stack_three[i] = self.word_vocab['<ROOT>']
            elif word in self.word_vocab:
                stack_three[i] = self.word_vocab[word]
            elif pos_word == 'CD' or pos_word == 'NNP' or pos_word == 'NNPS':
                stack_three[i] = self.word_vocab['<' + pos_word[:3] + '>']
            else:
                stack_three[i] = self.word_vocab['<UNK>']

        for i in range(len(buffer_three)):
            word = words[buffer_three[i]]
            pos_word = pos[buffer_three[i]]
            if word is None:
                buffer_three[i] = self.word_vocab['<ROOT>']
            elif word in self.word_vocab:
                buffer_three[i] = self.word_vocab[word]
            elif pos_word == 'CD' or pos_word == 'NNP' or pos_word == 'NNPS':
                buffer_three[i] = self.word_vocab['<' + pos_word[:3] + '>']
            else:
                buffer_three[i] = self.word_vocab['<UNK>']

        for i in range(len(stack_three), 3):
            stack_three.append(self.word_vocab['<NULL>'])
        for i in range(len(buffer_three), 3):
            buffer_three.append(self.word_vocab['<NULL>'])

        return np.array(stack_three + buffer_three)
